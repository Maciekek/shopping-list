...
/**
 * Sharing controls for one list, one screen top to bottom: the public toggle
 * first (a public list needs no invitations, so the rest hides), then invite
 * by link (QR, share sheet) or e-mail, then who has access.
...
export function ShareListDialog({
  ...
  const publicUrl = list.share
    ? `${origin}/sharedList/${list.share.token}`
    : '';
  ...
  const isPublic = !!list.share?.token;
          ...
          >
            ...
            <div className="flex flex-none items-center gap-1">
              ...
              {(isOwner || m.userId === user.id) && (
                <RemoveButton
                  label={
                    m.userId === user.id ? t('rejectShare') : t('removeMember')
                  }
                  onClick={() =>
                    run(() => revokeAccessToList(m.userId, list.id))
                  }
                />
              )}
            </div>
